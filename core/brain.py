...
from core.offline_brain import OfflineBrain  # Importamos el cerebro local
...
"""
        ...
        self.config = types.GenerateContentConfig(system_instruction=self.instruccion_sistema)
    ...
    def think(self, prompt, image=None):
        """Intenta online, cae a offline si falla."""
        if not self.use_offline:
            try:
                return self._think_online(prompt, image)
            except Exception as e:
                print(f"[Brain] Error online: {e}. Activando modo supervivencia...")
                self.use_offline = True
                return self.offline_brain.think(prompt)
        else:
            return self.offline_brain.think(prompt)

    def _think_online(self, prompt, image=None):
        max_retries = 2
        for intento in range(max_retries):
            try:
                if image:
                    response = self.client.models.generate_content(
                        model=self.model_id, contents=[prompt, image], config=self.config
                    )
                else:
