...
class Jarvis:
    def __init__(self):
        ...
        # 2. Interfaces y Cerebro (Carga de IA Local)
        self.listener = AudioListener(self.logger)
        self.listener.initialize()  # Carga de Whisper en CPU
        
        self.voice = VoiceSynthesizer()
        self.brain = Brain()
        
        # 3. Motores Avanzados (Autonomía y Flujos)
        ...
        # 5. Inicialización: Daemon Proactivo (Iniciativa)
        self.proactive_daemon = ProactiveDaemon(self.logger, self.event_bus, self.task_queue)
        self.proactive_daemon.add_monitor(SystemResourceMonitor(cpu_threshold=85, ram_threshold=90))
        self.proactive_daemon.add_monitor(InternetMonitor(interval=20))
        self.proactive_daemon.add_monitor(BatteryMonitor(low_threshold=25))
        # Recordatorio base de arranque
        self.proactive_daemon.reminders.add(9, 0, "Buenos días, Carlos. He iniciado el monitoreo de los sistemas.")
    ...
    def _on_system_ready(self, data):
        self.voice.speak("Sistemas integrados. Microfonía neuronal activa. Esperando órdenes.")
    ...
    def process_command(self, comando, attachment_path=None):
        ...
        # --- ENRUTAMIENTO ESTÁNDAR ---
            ...
            # Validación por si el micrófono no captó nada
    ...
    def run(self):
        # 1. Iniciar enlaces satelitales
        self.discord_link.run_in_background()
        self.logger.info("Enlace satelital de Discord iniciado.")
        
        # 2. Iniciar el Daemon Proactivo
        self.proactive_daemon.start()
        
        # 3. Anunciar que el sistema está listo
        self.event_bus.publish("SYSTEM_READY")
        
        # 4. Loop Principal de Escucha
        while True:
            texto = self.listener.listen(activo=False)
            
            if texto:
                # Normalizamos la transcripción para asegurar compatibilidad
                texto = texto.lower()
                
                if "salir" in texto or "apágate" in texto:
                ...
                if "jarvis" in texto:
                    # Extraer el comando real descartando la palabra clave
                    comando = texto.replace("jarvis", "").strip()
                    
                    # Si solo dijo "Jarvis" y hubo silencio
                    if not comando:
